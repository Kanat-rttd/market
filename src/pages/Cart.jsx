...
const Cart = () => {
    ...
    const { cart, totalPrice, totalQuantity, removeFromCart, increaseQuantity, decreaseQuantity } = useContext(CartContext)

    return (
        <div className='container mx-auto-8 min-h-96 px-4 md:px-16 lg:px-24'>
            {cart.length > 0 ?
                <div>
                    ...
                    <div className='flex flex-col md:flex-row justify-between space-x-10 my-8'>
                                ...
                                {cart.map((product) => (
                                    <div key={product.id} className='flex items-center justify-between p-3 border-b'>
                                        <div className='md:flex items-center space-x-4'>
                                            <img src={product.image} alt="" className='w-28 h-28 object-contain rounded' />
                                        ...
                                        <div className='flex space-x-12 items-center'>
                                            ...
                                            <div className='flex items-center justify-center border'>
                                                <button className='text-xl font-bold px-1.5 border-r' onClick={() => decreaseQuantity(product.id)}>-</button>
                                                <button className='text-xl px-2'>{product.quantity}</button>
                                                <button className='text-xl px-1 border-l' onClick={() => increaseQuantity(product.id)}>+</button>
                                            </div>
                        ...
                        <div className='md: w-1/3 bg-white p-6 mb-8 rounded-lg shadow-md border'>
                            ...
                            <div className='mb-4 border-b pb-2'>
                                ...
                                <button className='text-blue-500 hover:underline mt-1 ml-2' onClick={() => setIsModelOpen(true)}>
                                    Change address
                                </button>
                            </div>
